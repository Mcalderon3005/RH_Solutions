...
import 'schema/reporte_errores_record.dart';
import 'dart:async';
...
export 'schema/reporte_errores_record.dart';
...
/// Functions to query ReporteErroresRecords (as a Stream and as a Future).
Future<int> queryReporteErroresRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      ReporteErroresRecord.collection,
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<ReporteErroresRecord>> queryReporteErroresRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
    queryCollection(
      ReporteErroresRecord.collection,
      ReporteErroresRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<ReporteErroresRecord>> queryReporteErroresRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future<FFFirestorePage<ReporteErroresRecord>> queryReporteErroresRecordPage({
  Query Function(Query)? queryBuilder,
  DocumentSnapshot? nextPageMarker,
  required int pageSize,
  required bool isStream,
  required PagingController<DocumentSnapshot?, ReporteErroresRecord> controller,
  List<StreamSubscription?>? streamSubscriptions,
}) =>
    queryCollectionPage(
      ReporteErroresRecord.collection,
      ...
      if (isStream) {
        final streamSubscription =
            (page.dataStream)?.listen((List<ReporteErroresRecord> data) {
          data.forEach((item) {
            final itemIndexes = controller.itemList!
                .asMap()
                .map((k, v) => MapEntry(v.reference.id, k));
